...
Der atomare Datentyp in R ist der Vektor. Ein Vektor besteht aus einem oder mehreren Elementen. Demnach ist also auch eine einfache Zahl tatsächlich ein Vektor (mit einem Element). Ein Vektor mit mehreren Elementen wird mit dem Befehl `c` erzeugt (steht für combine oder concatenate).
...
Es gibt außerdem spezielle numerische Vektoren, welche nur aus Ganzzahlen bestehen (`integer`). In der Praxis werden diese aber eher selten eingesetzt.
...
### Logische Vektoren
Logische Vektoren bestehen aus den Werten `TRUE` oder `FALSE`. Sie sind vom Typ `logical`. In R kann man den Typ eines Vektors mit dem Befehl `class` herausfinden:
...
In R gibt es folgende Vergleichsoperatoren: `>`, `>=`, `<`, `<=`, `==` und `!=`. Vergleiche können mit `|` (oder) und `&` (und) verknüpft und mit `!` negiert werden. Gruppierungen durch Klammersetzung ist ebenfalls möglich.
```{r}
(3 > 5) & (4 == 4)
...
Achtung: Der Gleichheitsoperator lautet `==` (also zwei Gleichheitszeichen) und nicht `=`!
...
Die Funktion `length` gibt die Länge des Vektors aus und nicht die Anzahl der Zeichen einer Zeichenkette. Dafür kann man die Funktion `nchar` verwenden.
...
## Zwang (Coercion)
Vektoren sind homogene Datentypen, d.h. sie enthalten nur Elemente desselben Typs. Wenn man einen Vektor mit Elementen unterschiedlichen Typs erstellt, wird der Vektor automatisch in einen Typ gezwungen, welcher alle Elemente abbilden kann. Wenn man also Zahlen mit Zeichenketten mischt, werden alle Elemente in Zeichenketten umgewandelt (da Zeichenketten im allgemeinen nicht als Zahlen dargestellt werden können).
...
Wie erwähnt kann man mit `class` den Typ eines Vektors herausfinden. Mit folgenden verwandten Funktionen kann man überprüfen, ob es sich bei einem Vektor um einen bestimmten Typ handelt:
...
Der skalare Vektor 2 wird automatisch auf den Vektor `c(2, 2, 2, 2)` erweitert, daher entspricht die obige Operation eigentlich folgender Berechnung:
...
Hier kann man auch die Bedeutung der Werte in eckigen Klammern erkennen, die vor jeder Ausgabezeile stehen: sie geben den Index des ersten Elements der Zeile an (also im Beispiel `[1]` für die erste Zeile und `[54]` für die zweite Zeile).
...
### Übung 2
Erstellen Sie einen Vektor `x` mit den Elementen 4, 18, -7, 16, 4, 29, 8 und -44. Geben Sie `x` am Bildschirm aus. Erstellen Sie danach einen Vektor `y`, welcher die quadrierten Elemente aus `x` enthält (nutzen Sie dazu die Eigenschaft, dass R Rechenoperationen elementweise durchführt). Geben Sie `y` am Bildschirm aus. Zum Schluss erstellen Sie einen Vektor `z`, indem Sie `x` und `y` aneinander hängen. Geben Sie `z` am Bildschirm aus. Mit welchem Befehl können Sie die Anzahl der Elemente in `z` bestimmen?
