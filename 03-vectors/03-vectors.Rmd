...
# Creating vectors
A vector is a collection of elements. Importantly, vectors are atomic data types in R &ndash; that is, a vector is the most basic data type in R, and even a single number is represented as a vector. We can use the function `c` (combine, concatenate) to create a vector consisting of multiple elements:
...
In fact, `c` simply concatenates all its arguments, no matter if these are vectors with multiple elements themselves:
...
As a sidenote, it is possible to abbreviate `TRUE` and `FALSE` with `T` and `F`, respectively. However, it is discouraged to use these short names because it decreases readability.
...
Make sure to use `==` if you want to test for equality and not `=` (which is an assignment operator, and of course you should be using `<-` for that anyway).
...
# Coercion
Recall that vectors are homogenous data types (all elements have the same type). For example, all elements of a numeric vector are numbers. If we use the `c` function to create a vector with different types, R automatically coerces these elements to a vector that can represent them all instead of throwing an error. This means that mixing numbers and characters results in a character vector, and mixing numbers and logicals results in a numeric vector:
...
For example, a character vector can be converted into a numeric vector if its elements can be represented (interpreted) as numbers:
...
If recycling is not possible exactly because the length of the longer vector is not an integer multiple of the length of the shorter vector, R issues a warning (but recycling still works as we have just learned):
...
That's when we can finally solve the mystery of `[1]` that we see in front of every output: the number in square brackets corresponds to the index (the position in the vector) of the element right next to it. In the example, this means that the first `0` in the first line corresponds to index 1, whereas the first `0` in the second line corresponds to index 49 (the forty-ninth element in the vector). This is a convenient feature because we can quickly determine indexes in long vectors without having to count from one.
...
Finally, logical vectors are also permitted as index vectors. Here, the indexed vector consists of elements where the index vector is `TRUE`. This is useful because we can use comparisons (which result in logical vectors) to filter out specific elements of a vector:
...
# Named vectors
Optionally, elements of a vector can be named, which allows us to access individual elements not only by index (position), but also by name. We create named vectors by passing argument names and values in the function call to `c`:
...
## Exercise 2
Create a vector `x` consisting of elements 4, 18, -7, 16, 4, 29, 8, and -44. Then generate a vector `y` which contains the squared elements in `x`. Finally, create a vector `z` by combining/concatenating `x` and `y`. How can you determine the number of elements in `z`?
...
*Hint:* When dividing even numbers by two the remainder is zero. Odd numbers have a remainder of one. Use the remainder operator `%%` to compute these results.
...
- All elements of `k` except the eigth and ninth.
...
Use the `mean` function to compte the arithmetic mean. What is the result and how can you ignore the missing value?
