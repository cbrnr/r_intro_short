...
Man kann versuchen, die Funktion `read.table` mit Standardargumenten aufzurufen und dann Argumente so lange anpassen, bis die Daten korrekt eingelesen werden:

```{r}
read.table("lecturer.dat")
```

Das Ergebnis sieht gar nicht so falsch aus, nur die erste Zeile, die in der Datei die Variablennamen enthält, wurde fälschlicherweise als Datenpunkt erkannt. Dies können wir ändern, indem wir das zusätzliche Argument `header=TRUE` setzen:

```{r}
read.table("lecturer.dat", header=TRUE)
```

Diesmal sieht das entstandene Data Frame korrekt aus - wir können es direkt einer Variablen zuweisen, um damit weiterarbeiten zu können:
...
Die Hauptfunktion zum Einlesen von generischen Textdateien mit `readr` ist `read_delim`. Für CSV-Dateien gibt es den spezialisierte Wrapper `read_csv`, für mit Tabulatoren getrennte Daten heißt die Funktion `read_tsv` (bzw. `read_table`). Die letztgenannten Funktionen rufen aber lediglich `read_delim` mit geeigneten Argumenten auf.
...
## Daten aus SPSS
Sollen bereits vorhandene SPSS-Daten (.sav) importiert werden, kann man dazu die Funktion `read_sav` aus dem Paket `haven` verwenden. Das Ergebnis ist ein Tibble (da `haven` Teil des Tidyverse ist). Das `haven`-Paket kann übrigens auch Daten auch SAS und Stata importieren.
...
Bei Verwendung der Tidyverse-Funktionen ist die `name`-Spalte bereits im passenden Typ `character`, aber auch hier sollten die Spalten `birth_date` und `job` in geeignetere Typen umgewandelt werden:
```{r message=FALSE}
...
In diesem Beispiel bedeutet der Formatstring `%m/%d/%Y`, dass die Werte in der Reihenfolge Monat/Tag/Jahr vorliegen und mit einem Schrägstrich getrennt sind (`%Y` steht hier für eine vierstellige Jahreszahl).

### Zeichenketten
Die Spalte `name` wurde fälschlicherweise als Faktor interpretiert. Die Funktion `as.character` erzeugt daraus einen String-Vektor.
```{r}
...
```{r}
df <- data.frame(Person=c("Bob", "Al", "Sue"), Age=c(32, 24, 64), Weight=c(98, 61, 87), Height=c(188, 176, 174))
df
```
Das Paket `reshape2` stellt Funktionen zum Konvertieren von Wide nach Long (`melt`) und von Long nach Wide (`dcast`) zur Verfügung. Sehen wir uns zuerst die erste Möglichkeit an, nämlich die Umwandlung vom Wide-Format ins Long-Format:

```{r}
library(reshape2)
df_long <- melt(df, id.vars="Person", measure.vars=c("Age", "Weight", "Height"))
df_long
```

Hier spezifiziert man im Argument `id.vars` die Variablen (Spalten), welche eine Messung eindeutig identifizieren. In unserem Fall ist das die Spalte `Person`. Das Argument `measure.vars` spezifiziert alle Variablen, die Messwerte enthalten - im Beispiel also die Spalten `Age`,  `Weight` und `Height`, welche man in einen Vektor kombiniert zusammen übergibt.

Der zweite Fall, nämlich die Umwandlung vom Long-Format ins Wide-Format, wird durch die Funktion `dcast` gelöst:

```{r}
df_wide <- dcast(df_long, Person ~ variable, value="value")
df_wide
```
Hier übergibt man zuerst die Daten im Long-Format. Danach folgt eine Formel, welche die Umwandlungsregeln definiert. Auf der linken Seite der Formel (also links vom Tilde-Symbol) befinden sich alle Variablen, welche eine Messung eindeutig identifizieren (mehrere Variablen verbindet man mit dem `+`-Operator). Auf der rechten Seite der Formel (also rechts vom Tilde-Symbol) gibt man alle Variablen an, welche die Bezeichnungen der Messwerte beinhalten. Zu guter Letzt gibt man mit dem Argument `value` noch die Spalte an, welche die Messwerte beinhaltet.

Das Umformen von Daten nimmt einen großen Stellenwert im Tidyverse ein, daher gibt es auch eine Menge an nützlichen Funktionen um Daten ins gewünschte Format zu bringen (vor allem mit den Paketen `tidyr` und `dplyr`). Eine Beschreibung dieser Pakete würde aber den Rahmen dieser Veranstaltung sprengen.
...
*Hinweis:* Es ist wichtig, dass R beim Einlesen der Daten fehlende Werte korrekt erkennt. In dieser Datei werden fehlende Werte durch `?` markiert. Stellen Sie durch Setzen des entsprechenden Argumentes der Funktion `read.table` bzw. `read_delim` sicher, dass diese fehlenden Werte richtig eingelesen werden.